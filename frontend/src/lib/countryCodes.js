// Country calling codes for phone-number inputs (ISO 3166-1 alpha-2 + E.164 dial code).
// Sorted alphabetically by country name; United States listed first as the default.
...
// Dial codes ordered longest-first so a greedy prefix match picks the most
// specific code (e.g. "+1" shared by US/CA/many Caribbean nations still
// works since local number length differs, but codes like "+1242" don't
// exist here - this project only stores the E.164 dial code, not area code).
const DIAL_CODES_BY_LENGTH = [...new Set(COUNTRY_CODES.map(c => c.dial))]
  .sort((a, b) => b.length - a.length);

// Splits a stored phone string ("+91 98765 43210") into { dial, rest }.
// Falls back to the US dial code when nothing matches or the string is empty.
export function splitPhone(phone) {
  const trimmed = (phone || '').trim();
  if (!trimmed) return { dial: '+1', rest: '' };
  for (const dial of DIAL_CODES_BY_LENGTH) {
    if (trimmed.startsWith(dial)) {
      return { dial, rest: trimmed.slice(dial.length).trim() };
    }
  }
  return { dial: '+1', rest: trimmed };
}

export function joinPhone(dial, rest) {
  const trimmedRest = (rest || '').trim();
  return trimmedRest ? `${dial} ${trimmedRest}` : '';
}
