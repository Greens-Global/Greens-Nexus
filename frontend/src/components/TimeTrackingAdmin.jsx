...
const winScript = (url, token) => [
  ...
  //    Stop-Process (not taskkill): -ErrorAction SilentlyContinue is honored even
  //    under $ErrorActionPreference=Stop, and it writes no native stderr that
  //    would otherwise abort the script when a process isn't running.
  'Get-Process -Name "Greens Nexus Agent","GNAgent" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue',
...
// ── Disclosed-monitoring: central policy for what's captured, set by an admin. ──
// Each toggle is one plain sentence so it's clear what turning it on records.
const MON_TOGGLES = [
  ['enabled',      'Monitoring enabled',       'Master switch — turns disclosed monitoring on for everyone clocked in.'],
  ['trackScreens', 'Capture screens',          'Periodic screenshots of the work screen while the person is clocked in.'],
...
export default function TimeTrackingAdmin() {
  ...
  // Disclosed-monitoring policy (loaded + saved here — this is the central control).
  ...
  return (
    <div style={{ fontFamily: 'Inter,sans-serif', maxWidth: 860, margin: '0 auto' }}>
      {/* Monitoring policy — disclosed-monitoring: central control of what's captured. */}
      <div style={{ border: '1px solid var(--line)', borderRadius: 14, padding: '16px 18px', marginBottom: 22, background: 'var(--card)' }}>
        ...
        <p style={{ margin: '0 0 14px', fontSize: 12, color: 'var(--muted)', lineHeight: 1.55 }}>
          ...
          the first time they clock in each day. Changes take effect the next time a device reads its status.
        </p>
        {policy === null ? (
