...
export const fieldsForProject = (customFields, projectId) =>
  (customFields || []).filter((f) => {
    // A field can apply to tasks, projects, or both (appliesTo is a list) -
    // only the ones that include 'task' show as a task column. Missing
    // appliesTo = a field created before project-scoped fields existed.
    if (!(f.appliesTo || ['task']).includes('task')) return false;
    const ids = (f.projectIds || []).filter(Boolean);
    return ids.length === 0 || (projectId ? ids.includes(projectId) : false);
  });

// The custom fields that apply to the project entity itself (Location, etc.)
// - these render on the project record (ProjectsView), not as a task column.
// A field can be in both lists at once if it applies to both.
export const fieldsForProjectEntity = (customFields) =>
  (customFields || []).filter((f) => (f.appliesTo || []).includes('project'));
