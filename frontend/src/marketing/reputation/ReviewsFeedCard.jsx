...
import { filterReviews } from './aggregate'
import { C } from '../theme'
...
export default function ReviewsFeedCard({
  reviews, onApprove, onRegenerate,
  ...
  const filtered = useMemo(
    () => filterReviews(reviews, { status: statusFilter, platform: platformFilter, rating: ratingFilter, query }),
  ...
  return (
    <div style={{ borderRadius: 12, border: '1px solid ' + C.gray200, background: C.white, padding: 16, boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', maxHeight: 720, display: 'flex', flexDirection: 'column', gap: 12, paddingRight: 4 }}>
        ...
        {filtered.map((r) => (
          <ReviewCard key={r.id} review={r} onApprove={onApprove} onRegenerate={onRegenerate} highlightWord={query} />
        ))}
      </div>
    </div>
  )
