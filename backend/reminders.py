    ...
    for g in db.query(NexusGroup).all():
        if "hr:" in (g.allowed_modules or ""):
            for m in (
                ...
                .filter(NexusGroupMember.group_id == g.id)
                .all()
            ):
...
def run_daily_scan() -> int:
    ...
    try:
        ...
        employees = (
            ...
            .filter(NexusEmployee.status.in_(["active", "onboarding"]))
            .all()
        )
        ...
        for e in employees:
            ...
            if d is not None and d in (7, 3, 1, 0):
                ...
                for r in set(
                    hr_team + ([e.manager_email.lower()] if e.manager_email else [])
                ):
                    sent += _notify(
                        db,
                        "hr_new_starter",
        ...
        for c in db.query(HrCandidate).filter(HrCandidate.interview_at != "").all():
            if (c.interview_at or "")[:10] == today and c.stage not in (
                "hired",
                "rejected",
            ):
        ...
        if _SCAN_HOUR_UTC - 1 <= now_utc.hour <= _SCAN_HOUR_UTC + 2:
            ...
            for e in employees:
                ...
                try:
                    ...
                    ysum = _day_summaries(
                        _live_punches(db, e.work_email, yesterday, today)
                    ).get(yesterday, {})
                    if "missing_out" in (ysum.get("flags") or []):
                        msg = (
                            f"{name} clocked in on {yesterday} but never clocked out - "
