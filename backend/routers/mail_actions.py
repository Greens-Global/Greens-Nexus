...
# ── Microsoft Actionable Message token ──────────────────────────────────────
# https://learn.microsoft.com/outlook/actionable-messages/security-requirements
# and .../enable-entra-token-for-actionable-messages
# The Actions service's own app id - the client Outlook's action requests come
# from. It is preauthorized on our app registration ("Expose an API" -> Add a
# client application), and the token names it in `azp`.
AM_APP_ID = "48af08dc-f6d2-435f-b2a7-069abd99c086"
# Audience(s): the Application ID URI generated by the Actionable Messages
# provider registration (api://auth-am-<guid>/<guid>) and set on our Entra app
# - the `aud` of a v1 token - and, comma-separated after it, that app's client
# id, which is what a v2 token carries as `aud` instead. Which version Entra
# mints depends on the endpoint the Actions service uses, so both are accepted
# (task_mail_actions.AM_AUDIENCE holds the raw setting; the card is only
# emitted when it is present).


def _am_audiences() -> list[str]:
    return [a.strip() for a in (tma.AM_AUDIENCE or "").split(",") if a.strip()]


def _am_issuers() -> set[str]:
    """Our tenant, in both spellings Entra uses: v2 tokens carry
    login.microsoftonline.com/<tid>/v2.0, v1 tokens sts.windows.net/<tid>/."""
    return {auth.ISSUER, f"https://sts.windows.net/{auth.TENANT_ID}/"}
...
def performer_from_claims(claims: dict, senders: set[str]) -> str:
    """The email of the person who clicked, from a VERIFIED token's claims.

    Entra ID tokens (see am_performer): `azp` names the calling app, `tid` the
    tenant, and the user is `preferred_username` - NOT `sub`, which is an
    opaque pairwise identifier here, unlike the retired EAT token where `sub`
    was the address itself. `sender` only rides along on EAT-era tokens; when
    it is present it must be one of our own mailboxes, and when it is absent
    the signed per-task token in the URL is what ties the call to an email we
    sent. Separated from the decode above so the rules are testable without
    minting a signed token."""
    if claims.get("azp") != AM_APP_ID and claims.get("appid") != AM_APP_ID:
        raise HTTPException(401, "Token was not issued for Actionable Messages")
    tenant = (claims.get("tid") or "").strip()
    if tenant and auth.TENANT_ID and tenant != auth.TENANT_ID:
        raise HTTPException(401, "Token is from another tenant")
    sender = (claims.get("sender") or "").strip().lower()
    ...
    for claim in ("preferred_username", "upn", "email", "unique_name", "sub"):
        who = str(claims.get(claim) or "").strip().lower()
        if "@" in who:
            return who
...
def am_performer(authorization: str, db) -> str:
    ...
    for anything that is not a genuine Actionable Message request.

    Microsoft retired the legacy (EAT) token on June 8, 2026 - action requests
    now carry a Microsoft Entra ID token, so this validates against OUR tenant's
    issuer and keys (the same ones auth.py already uses for the SPA) with the
    audience set to the Application ID URI from the provider registration.
    Validating the old substrate.office.com issuer would reject every real
    click today.
    """
    if not _am_audiences():
        raise HTTPException(503, "In-mail actions are not configured on this deployment "
                                 "(NEXUS_AM_AUDIENCE is unset).")
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(401, "Missing Actionable Message token")
    token = authorization.removeprefix("Bearer ").strip()
    try:
        # Issuer is checked by hand below: PyJWT takes one issuer, Entra has two.
        claims = pyjwt.decode(token, auth._get_public_key(token), algorithms=["RS256"],
                              audience=_am_audiences(), options={"verify_iss": False})
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(401, f"Invalid Actionable Message token: {exc}")
    if claims.get("iss") not in _am_issuers():
        raise HTTPException(401, "Token is from another tenant")
    return performer_from_claims(claims, _sender_mailboxes(db))
